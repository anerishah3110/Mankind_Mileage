...
function validate_password(){
  ...
  function loadState()
		...
		xmlhttp.onreadystatechange = function() {
			if (xmlhttp.readyState == 4) {
				...
				for(var i=0 ; i<jsonObj.length ; i++)
				{
					...
					document.cordblood.state_example.options.add(createOption);
	...
	{
		var removeState=document.cordblood.state_example.options.length;
		//alert(removeState);
		for(var i=removeState ; i>0 ; i-- )
		{
			//alert(document.cityForm.state_example.options[i].text);
			document.cordblood.state_example.remove(i);
		...
		xmlhttp.onreadystatechange = function() {
			if (xmlhttp.readyState == 4) {
				...
				for(var i=0 ; i<jsonObj.length ; i++)
				{
					...
					document.cordblood.city_example.options.add(createOption);
	...
	{
		var removeState=document.cordblood.city_example.options.length;
	//	alert(removeState);
		for(var i=removeState ; i>0 ; i-- )
		{
			//alert(document.cityForm.state_example.options[i].text);
			document.cordblood.city_example.remove(i);
...
<div id="main">
		...
		<h2>CordBloodStorage Registration</h2>

		<form onsubmit="return validate_password()" method="post" class="login" action="<%=request.getContextPath() %>/UserMasterController" name="cordblood">
		<input type="hidden" name="flag" value="insertcord">
		<input type="hidden" name="status" value="unblock">
		<input type="hidden" name="type" value="cordblood">
			...
			<p class="form-row form-row-wide">
				...
				<select name="country_example" id="country_example" onchange="loadState()" required="required">
			...
			<p class="form-row form-row-wide">
				...
				<select name="state_example" id="state_example" onchange="loadCity()" required="required">
				<option value="">----select----</option>
			...
			<p class="form-row form-row-wide">
				...
				<input type="date" class="input-text" name="duedate" id="duedate1" placeholder="DD/MM/YYYY" required="required"/>
			</p>
			<p class="form-row form-row-wide">
